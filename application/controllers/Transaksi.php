	...
	public function aksipenjualan()
	{
		...
		foreach ($_POST['kode_barang'] as $key => $value) {
			...
			if($sisa_stok < 0){ //jika stok < 0
				...
				$this->session->set_flashdata('sukses', '<div class="alert alert-success" role="alert">Berhasil Melakukan Transaksi '.' '.$kode_trans.''.' ! </div><div class="alert alert-danger" role="alert">Stok <strong>'.$stok[0]['nama_barang'].'</strong> Habis !</div>');
				redirect('transaksi/penjualan');
